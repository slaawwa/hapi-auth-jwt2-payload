...
internals.implementation = function (server, options) {
  ...
  const _authFN = function (request, reply) {

    var token = extract(request, options); // extract token Header/Cookie/Query/OR_SECOND_PAYLOAD
    var tokenType = options.tokenType || 'Token'; // see: https://git.io/vXje9
    var decoded, keyFunc;
    var isPOST = request.auth.credentials == 'tryPost';
                ...
                return options.validateFunc(verify_decoded, request,
                  function (validate_err, valid, credentials) { // bring your own checks
                    ...
                    if (!valid) {
                      ...
                      request.auth.credentials = credentials || verify_decoded;
